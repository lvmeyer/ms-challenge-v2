...
export const ProductComments = () => {
	...
	const handleCommentSubmit = () => {
		fetch(import.meta.env.VITE_GW_HOSTNAME + '/api/v1/reviews', {
			...
			.then((response) => response.json())
			.then((data) => {
				setUsersComments((prevComments) => [...prevComments, data.data]);
				setCommentText('');
			});
	};

	const handleReportComment = (commentId) => {
		const alreadyReported = comments.some(
			(comment) => comment.id === commentId && comment.isReported
		);
		if (alreadyReported) {
			return;
		}
		...
		fetch(
			import.meta.env.VITE_GW_HOSTNAME + `/api/v1/reviews/report/${commentId}`,
			{
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json',
						...
						<div className="one-comment comment" key={usersComment.id}>
							<div>
								<div className="author-date">
									...
									<div className="comment-date">
										{renderStars(usersComment.rating)}
									</div>
								</div>
							...
							<div>
								<button
									...
									onClick={() => handleReportComment(usersComment.id)}
								>
									Report
								</button>
							</div>
